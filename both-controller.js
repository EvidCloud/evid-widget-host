...
(function () {
  ...
  function getThisScriptEl() {
    ...
    // Fallback: last script that contains "both-controller"
  ...
  (async function boot() {
    ...
    /* =========================================
       1) FIREBASE (aligned with your firebase-config.js)
       ========================================= */
    ...
    // Prevent double-mount on same element
    ...
    /* =========================================
       3) SETTINGS (defaults + Firestore + <script data-*>) 
       ========================================= */
    ...
    try {
      ...
      if (widgetId) {
        ...
        if (docSnap.exists()) {
          ...
          // delay (prefer seconds if small; accept ms)
          if (readAny(s, ["delay", "delaySeconds", "initDelay", "initDelayMs"]) !== undefined) {
            ...
            if (Number.isFinite(raw)) {
              // If it looks like seconds (0-300), treat as seconds
              DYNAMIC_SETTINGS.delay = raw <= 300 ? raw * 1000 : raw;
            } else {
              DYNAMIC_SETTINGS.delay = 0;
            }
          }
    ...
    // ---- <script data-*> overrides (only if Firestore didn't explicitly provide that field) ----
    try {
