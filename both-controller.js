/*! both-controller v4.2.1 — FULL FEATURE SET (Explicit Attributes) */
(function () {
  ...
  /* =========================================
     1. קריאת הגדרות (Explicit Attributes)
     ========================================= */
  ...
  var PAGE_TRANSITION_DELAY = 3000; // הדיליי במעבר עמודים
  var STORAGE_KEY = 'evid:widget-state:v5'; // מפתח חדש לניקוי זיכרון ישן

  // בדיקת תקינות בסיסית
  if (!REVIEWS_EP && !PURCHASES_EP) {
    // אם אין שום מקור מידע, לא מציגים כלום (שקט תעשייתי)
    return;
  }

  /* =========================================
     2. מילון שמות (זיהוי מגדרי חכם)
     ========================================= */
  ...
  function getActionText(name, defaultAction) {
    ...
    if (FEMALE_NAMES.has(firstName)) {
      ...
      return defaultAction + " (ה)"; // ברירת מחדל אם הפועל לא מוכר
    }
    
    // לשון זכר (מסירים את ה- "/ה")
    return defaultAction.replace("/ה", "");
  }

  /* =========================================
     3. טעינת פונטים ועיצוב (CSS)
     ========================================= */
  ...
  /* FIX: Padding left to avoid overlap with X button */
  ...
  /* Purchases: Full Width Strip with Margin */
  ...
  /* Normalize Data (CRITICAL FIX FOR buyerName) */
  function normalizeArray(data, as){
    ...
    return arr.map(function(x){
      ...
      if(as==="purchase") {
        // Here we handle both "buyer" and "buyerName"
  ...
  function interleave(reviews, purchases){
    // Determine which queue based on presence of Endpoints
    var hasRev = !!REVIEWS_EP;
    var hasPur = !!PURCHASES_EP;
    
    if (hasRev && !hasPur) return reviews;
    if (hasPur && !hasRev) return purchases;
  ...
  function resumeFromReadMore(){
    ...
    scheduleHide(SHOW_MS); // Restart timer for simplicity
  ...
  function renderPurchaseCard(p, overrideTime){
    ...
    // Custom Live Text + Unified Badge
