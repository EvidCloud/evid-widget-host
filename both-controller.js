...
+ ".card.style-leaf.review-card.compact.has-readmore { padding-bottom: 8px !important; }"
      ...
      + ".evid-mini-icon { width: 22px; height: 22px; display: inline-block; }"
...
const EXTRA_PAD_PX = 32; // תנסה 10–18 לפי מה שנוח
    ...
    const update = () => {
      ...
      shift = Math.max(-MAX_DOWN_PX, shift);
       ...
       const extraPad = shift < -1 ? EXTRA_PAD_PX : 0; // רק כשאנחנו "יורדים" (בר נעלם)
wrapEl.style.setProperty("--evid-ios-extra-pad", extraPad + "px");
